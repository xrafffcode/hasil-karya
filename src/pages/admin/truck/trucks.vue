...
<template>
  ...
  <VRow>
    <VCol cols="12" class="d-flex justify-space-between align-items-center">
      ...
      <VBtn :to="{ name: 'admin-truck-create' }" color="primary">
        Tambah Truck
      </VBtn>
    </VCol>

    <VCol cols="12">
      <VTextField v-model="search" label="Cari Truck" placeholder="Cari Truck" clearable :loading="loading"
        variant="solo" />
    </VCol>

    <VCol cols="12">
      <VCard>
        <EasyDataTable :headers="headers" :items="trucks" :loading="loading" :search-value="search" buttons-pagination
          show-index>
          <template #loading>
          ...
          <template #item-is_active="item">
            <VSwitch v-model="item.is_active" color="primary" @change="() => handleActivateTruck(item)" />
          </template>
          <template #item-operation="item">
            <VBtn :to="{ name: 'admin-truck-edit', params: { id: item.id } }" color="primary" size="small" class="m-5">
              Ubah
            </VBtn>
            <VBtn :to="{ name: 'admin-truck-view', params: { id: item.id } }" color="info" size="small">
              Detail
            </VBtn>
            <VBtn color="error" size="small" class="m-5" @click="() => handleDeleteTruck(item)">
              Hapus
            </VBtn>
          </template>
        </EasyDataTable>
